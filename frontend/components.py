...
def render_app_header(doc_count: int, llm_online: bool = True) -> None:
    doc_text = f"{doc_count} PDF{'s' if doc_count != 1 else ''}" if doc_count > 0 else "0 PDFs"
    st.markdown(f"""
        <div style="display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid var(--border);padding-bottom:0.75rem;margin-bottom:1.5rem">
            <h3 style="margin:0;font-weight:700;color:var(--text-primary);font-size:1.2rem">🧠 PaperBrain Chat</h3>
